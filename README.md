...
## Overview
This repository provides a lightweight TypeScript project with source files in `src/`, tests in `test/`, and compiled output in `dist/`. It uses Vitest for tests and coverage, TypeScript for builds, and ESLint flat config with `typescript-eslint`.
...
- **build**: Compiles TypeScript to CommonJS in `dist/`.
  ...
  yarn build
  ```
- **clean**: Removes generated build and coverage output.
  ```bash
  yarn clean
  ```
